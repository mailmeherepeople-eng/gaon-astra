# Performance: 8 September 2026 repair release
...
- Each neem's trunk and branches are merged into one mesh before the tree is cloned, so a tree costs two draws instead of seventeen. The banyan's branches and aerial roots are merged the same way; its leaves are one instanced mesh.
...
Headless Edge 152.0.4191.66 on the development computer; phone emulation is 390×844 at device scale 3 with Low quality (render DPR 1), desktop is 1440×900 at device scale 1 with High quality. Each scenario warms for 900 ms and samples 90 animation frames. Raw camera, DPR, median/p95 and resource values are in [benchmark-results.json](audits/fixes-2026-09-08/benchmark-results.json).

| Profile | Scenario | Median calls | Median triangles |
| --- | --- | ---: | ---: |
| phone emulation | start | 108 | 102,258 |
| phone emulation | pump | 101 | 100,868 |
| phone emulation | crowded | 325 | 130,387 |
| phone emulation | night | 163 | 110,165 |
| phone emulation | interior | 54 | 3,606 |
| desktop | start | 1,047 | 408,646 |
| desktop | pump | 968 | 397,104 |
| desktop | crowded | 1,640 | 634,507 |
| desktop | night | 990 | 370,571 |
| desktop | interior | 110 | 18,754 |

The new starting location faces the pump and differs from the old house-door start, so that row is not an identical-camera before/after comparison. The crowded scenario deliberately groups the village population. Scene generation still contains randomness, and shadows add work on High; counts will vary between runs. The focused performance regression suite produced another phone pump observation of 106 calls / 108,880 triangles and desktop 1,096 calls / 501,736 triangles.

Recorded median/p95 animation intervals were around 6.9/7.2 ms on this computer. These describe the desktop display/browser scheduler and are **not a claim of 144 FPS on a phone**, GPU timing or sustained thermal performance. Resource counts can rise as previously unseen scenery is first uploaded; the carrying regression separately verifies no per-cycle growth after warm-up (906 → 906 over 30 cycles).
